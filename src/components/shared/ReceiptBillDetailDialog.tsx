...
import { getReceiptBill } from "@/services/receiptService";
...
const ReceiptBillDetailDialog = ({ isOpen, setIsOpen, billId }: Props) => {
  const { data, isLoading, isError } = useQuery({
    queryKey: ["receipt-bill-detail", billId],
    queryFn: ({ signal }) => getReceiptBill({ signal, billId }),
  });
        ...
        <div className="grid grid-cols-5 gap-4">
          <div className="col-span-3">
            <ul className="flex h-[30rem] flex-col gap-4 overflow-y-auto">
              ...
              {!isLoading && data && (
                <>
                  {data.receiptBillDetails.map((item) => {
                    return (
                      <li
...
export default ReceiptBillDetailDialog;
